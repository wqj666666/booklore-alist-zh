...
#### AList 存储增强
- **AList 书籍扫描与元数据提取**
  - 在 LibraryFileHelper 中实现 AList 目录的递归扫描
  - 修复无法发现远程文件的问题
  - 支持完整的 AList 文件系统遍历

- **虚拟哈希机制**
  - 基于 AList 文件元数据（路径、大小、修改时间）计算虚拟 MD5
  - 避免扫描时下载完整文件，大幅提升扫描效率
  - 支持增量扫描检测文件变更

- **文件处理器优化**
  - 在提取元数据和封面时将 AList 文件下载至临时目录处理
  - 处理完成后自动清理临时文件
  - 支持大文件流式下载

- **数据模型扩展**
  - 扩展 BookFileEntity 及数据库表结构
  - 新增 AList 路径、文件大小、修改时间等元数据字段
  - 支持本地与 AList 文件的统一管理

### 🐛 Bug 修复

- 修复 AList 存储模式下获取文件大小时触发的空指针异常
- 修复远程文件扫描时的路径解析问题
